...
import { LevelMenuItem, MenuItem } from "./menuItem";
...
export class VerticalScrollMenu extends Entity {
  ...
  addMenuItem(_item: MenuItem) {
    ...
    const idx = this.itemRoots.length
    ...
    this.replaceMenuItem(idx,_item)
  }

  replaceMenuItem(idx:number,_item:MenuItem){
    const clickBox = this.clickBoxes[idx]
    clickBox.addComponent(
      new OnPointerDown(
        (e) => {
          ...
          // click to select
          if (e.buttonId == 0) {
            if (!_item.selected || _item.reSelectCooldownPassed()) {
  ...
  removeMenuItem(index: number) {
    if (index > -1) {
      //log("this.items[index]", this.items[index].isAddedToEngine());

      if (this.items[index].isAddedToEngine()) engine.removeEntity(this.items[index]);
      /*     log('this.itemRoots[index]', this.itemRoots[index].isAddedToEngine())
      if(this.itemRoots[index].isAddedToEngine()) engine.removeEntity(this.itemRoots[index]); */
      //log("this.clickBoxes[index]", this.clickBoxes[index].isAddedToEngine());
      if (this.clickBoxes[index].isAddedToEngine()) engine.removeEntity(this.clickBoxes[index]);
  ...
  selectItem(_item: MenuItem) {
    ...
    if (_item.hasComponent(AnimatedItem)) {
      const isLevelMenuItem = _item instanceof LevelMenuItem
      if (!_item.selected || _item.reSelectCooldownPassed()) {
        this.deselectAll();
