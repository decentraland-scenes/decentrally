...
class ItemAnimationSystem {
    ...
    for (let item of this.group.entities) {
      ...
      if (!info.isHighlighted) {
        if (info.animFraction > 0) {
          ...
          if (info.defaultTransform.scale && info.highlightTransform.scale)
            transform.scale = Vector3.Lerp(
              info.defaultTransform.scale,
              info.highlightTransform.scale,
              1 - this.easeOutExp(1 - info.animFraction)
            );
          if (info.defaultTransform.position && info.highlightTransform.position)
            transform.position = Vector3.Lerp(
              info.defaultTransform.position,
              info.highlightTransform.position,
              1 - this.easeOutExp(1 - info.animFraction)
            );
          if (info.defaultTransform.rotation && info.highlightTransform.rotation)
            transform.rotation = Quaternion.Slerp(
              info.defaultTransform.rotation,
              info.highlightTransform.rotation,
              1 - this.easeOutExp(1 - info.animFraction)
            );
          ...
          info.animFraction = 0;
          if (info.defaultTransform.scale) transform.scale = info.defaultTransform.scale;
          if (info.defaultTransform.position) transform.position = info.defaultTransform.position;
          if (info.defaultTransform.rotation) transform.rotation = info.defaultTransform.rotation;
        }
      } else {
        if (info.animFraction < 1) {
          ...
          if (info.defaultTransform.scale && info.highlightTransform.scale)
            transform.scale = Vector3.Lerp(
              info.defaultTransform.scale,
              info.highlightTransform.scale,
              this.easeOutExp(info.animFraction)
            );
          if (info.defaultTransform.position && info.highlightTransform.position)
            transform.position = Vector3.Lerp(
              info.defaultTransform.position,
          ...
          info.animFraction = 1;
          if (info.highlightTransform.scale) transform.scale = info.highlightTransform.scale;
          if (info.highlightTransform.position) transform.position = info.highlightTransform.position;
          if (info.highlightTransform.rotation) transform.rotation = info.highlightTransform.rotation;
        }
  ...
  springVec3(a_Target: Vector3, a_Current: Vector3, a_currentVelocity: number, a_TimeStep: number): Vector3 {
