  ...
  async update(dt: number) {
    ...
    if (this.ticker >= this.interval) {
      ...
      if (!realm || (!this.isPreview && !realm.room)) {
        ...
        await connect(CONFIG.GAME_LOBBY_ROOM_NAME).then(async (room: Room) => {
          onConnect(room)
        })
...
let myConnectSystem = undefined //new ConnectSystem()
if(myConnectSystem) engine.addSystem(myConnectSystem)
...
onRealmChangedObservable.add(async (realmData) => {
  if (realmData && realmData.room) {
    ...
    if(myConnectSystem) myConnectSystem.connected = true
    log('CONNECTING TO WS SERVER')
    await connect(CONFIG.GAME_LOBBY_ROOM_NAME).then(async (room: Room) => {
      onConnect(room)
    })
